...
const opportunitySchema = z.object({
  ...
  expectedOpportunityValue: z.number({ invalid_type_error: "Expected Deal Value must be a number" }).nonnegative("Expected Deal Value must be non-negative").optional().nullable(),
...
export default async function opportunityRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/opportunities/export", { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const csv = toCsv(rows, [
      ...
      { key: "opportunityType", label: "Deal Stage" },
      { key: "assignedTo", label: "Assigned To" },
      { key: "expectedOpportunityValue", label: "Expected Deal Value" },
