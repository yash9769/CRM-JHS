...
export default async function reportRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/reports/pipeline-health", { preHandler: [app.authenticate] }, async (req: any) => {
    ...
    const stageData = await Promise.all(stages.map(async (stage) => {
      const deals = await prisma.deal.findMany({
        where: { tenantId, stageId: stage.id, stage: { isClosed: false } },
    ...
    const data = await Promise.all(users.map(async (user) => {
      const [openDeals, wonDeals, lostDeals, openOpps] = await Promise.all([
        prisma.deal.findMany({
          where: { tenantId, ownerId: user.id, stage: { isClosed: false } },
          select: { amount: true, probability: true },
        }),
        prisma.deal.findMany({
          where: { tenantId, ownerId: user.id, wonDate: { gte: periodStart, lte: periodEnd } },
          select: { amount: true },
        }),
        prisma.deal.findMany({
          where: { tenantId, ownerId: user.id, stage: { isClosed: true, isWon: false }, updatedAt: { gte: periodStart, lte: periodEnd } },
          select: { amount: true },
        }),
        prisma.opportunity.count({ where: { tenantId, ownerId: user.id, isConverted: false } }),
      ]);
  ...
  app.get("/api/v1/reports/win-loss", { preHandler: [app.authenticate] }, async (req: any) => {
    ...
    const [wonDeals, lostDeals] = await Promise.all([
      prisma.deal.findMany({
        where: { tenantId, wonDate: { gte: since }, stage: { isClosed: true, isWon: true } },
        include: { stage: true, owner: { select: { id: true, firstName: true, lastName: true } } },
      }),
      prisma.deal.findMany({
        where: { tenantId, updatedAt: { gte: since }, stage: { isClosed: true, isWon: false } },
        include: { stage: true, owner: { select: { id: true, firstName: true, lastName: true } } },
      }),
    ]);
  ...
  app.get("/api/v1/reports/conversion-funnel", { preHandler: [app.authenticate] }, async (req: any) => {
    ...
    const stageCounts = await Promise.all(stages.map(async (s) => {
      const [count, amount] = await Promise.all([
        prisma.opportunity.count({ where: { tenantId, stageId: s.id } }),
        prisma.opportunity.aggregate({ where: { tenantId, stageId: s.id }, _sum: { amount: true } }),
      ]);
