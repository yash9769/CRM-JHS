...
async function seedTenant(tenant: any, mainUser: any) {
  console.log(`\n🌱 Seeding tenant: ${tenant.name} (${tenant.id})...`);

  // Ensure password for main user
  const passwordHash = await argon2.hash("Password123!");

  // Ensure mainUser exists and is ADMIN in this tenant if not set
  let user = mainUser;
  if (!user) {
    user = await prisma.user.findFirst({ where: { tenantId: tenant.id } });
  }
  if (!user) {
    user = await prisma.user.create({
      data: {
        tenantId: tenant.id,
        email: `admin@${tenant.name.toLowerCase().replace(/[^a-z0-9]/g, "")}.com`,
        passwordHash,
        firstName: "System",
        lastName: "Admin",
        role: "ADMIN",
  ...
  // Also ensure yashodhan.rajapkar and yash@gmail.com exist in this tenant if needed
  const extraEmails = ["yash@gmail.com", "yashodhan.rajapkar@envistacyberdefence.com"];
  for (const em of extraEmails) {
    const existing = await prisma.user.findFirst({ where: { email: em } });
    if (existing) {
      await prisma.user.update({
        where: { id: existing.id },
        data: { tenantId: tenant.id, passwordHash, active: true },
      });
    } else {
  ...
  // Refresh user ref
  user = await prisma.user.findFirstOrThrow({ where: { tenantId: tenant.id } });
  ...
  if (!oppPipeline) {
    oppPipeline = await prisma.pipeline.create({
      data: {
        tenantId: tenant.id,
        name: "Enterprise Opportunity Pipeline",
        type: "OPPORTUNITY",
        isDefault: true,
        stages: {
          create: [
  ...
  // 3. Products Catalog
  const productsData = [
    { name: "Enterprise Security Audit", sku: `SEC-AUD-${tenant.id.slice(0, 3)}`, category: "Consulting", unitPrice: 150000, description: "Comprehensive ISO27001 & SOC2 gap assessment and penetration audit." },
    { name: "Cloud Infrastructure Pentest", sku: `SEC-PEN-${tenant.id.slice(0, 3)}`, category: "Security Services", unitPrice: 85000, description: "AWS & Azure environment vulnerability and penetration testing." },
    { name: "Managed SOC Subscription (Annual)", sku: `SOC-SUB-${tenant.id.slice(0, 3)}`, category: "Managed Services", unitPrice: 500000, description: "24/7 SIEM monitoring, threat detection, and incident escalation." },
    { name: "Incident Response Retainer", sku: `IR-RET-${tenant.id.slice(0, 3)}`, category: "Retainer", unitPrice: 120000, description: "Guaranteed 1-hour SLA response for critical security incidents." },
    { name: "Security Awareness Employee Training", sku: `TRN-SEC-${tenant.id.slice(0, 3)}`, category: "Training", unitPrice: 45000, description: "Interactive anti-phishing and security awareness platform for staff." },
  ];
  ...
  const accountsData = [
    ...
    { name: "Reliance Digital Solutions", domain: "reliancedigital.in", industry: "Telecommunications", employeeCount: 5200, annualRevenue: 1200000000, accountType: "CUSTOMER" as const, phone: "+91-22-30004000", website: "https://reliancedigital.in", ownerId: createdTeam[Object.keys(createdTeam)[1]] || user.id },
    ...
    { name: "Infosys Security Labs", domain: "infosyssec.com", industry: "Software", employeeCount: 380, annualRevenue: 150000000, accountType: "PROSPECT" as const, phone: "+91-80-41102000", website: "https://infosyssec.com", ownerId: createdTeam[Object.keys(createdTeam)[2]] || user.id },
  ...
  // 6. Leads (Pre-qualified prospect pool)
  const leadsData = [
  ...
  const getOppStage = (name: string) => oppStages[name] || oppStages["Proposal / Quote"] || oppStages["Negotiation"] || oppPipeline.stages[0]?.id;
  const getDealStage = (name: string) => dealStages[name] || dealStages["Proposal Sent"] || dealStages["Proposal"] || dealPipeline.stages[0]?.id;

  // 7. Opportunities (Pipeline Kanban)
  const oppsData = [
    ...
    { name: "Reliance Multi-Cloud Pentest Project", accountId: relianceAcc.id, amount: 1200000, stageId: getOppStage("Executive Negotiation"), probability: 80, type: "NEW_BUSINESS" as const, ownerId: user.id },
    { name: "Tata Cloud ISO27001 Security Audit", accountId: tataAcc.id, amount: 850000, stageId: getOppStage("Technical Demo") || getOppStage("Demo / Presentation"), probability: 40, type: "NEW_BUSINESS" as const, ownerId: user.id },
    { name: "Infosys Annual SOC Monitoring", accountId: infosysAcc.id, amount: 2400000, stageId: getOppStage("Discovery Call") || getOppStage("Discovery"), probability: 25, type: "NEW_BUSINESS" as const, ownerId: user.id },
    { name: "Apex Logistics Perimeter Defense", accountId: apexAcc.id, amount: 350000, stageId: getOppStage("Qualified Lead") || getOppStage("Qualified"), probability: 10, type: "NEW_BUSINESS" as const, ownerId: user.id },
  ];
  ...
  // 8. Deals (Revenue Closed & Contracting)
  const dealsData = [
    ...
    { name: "HDFC SIEM & Threat Intelligence Deal", accountId: hdfcAcc.id, amount: 1800000, stageId: getDealStage("Legal & Security Review") || getDealStage("Contract Review"), probability: 90, forecastCategory: "COMMIT" as const, ownerId: user.id },
    { name: "Tata Cloud Security Architecture Retainer", accountId: tataAcc.id, amount: 950000, stageId: getDealStage("Contract Negotiation") || getDealStage("Negotiation"), probability: 70, forecastCategory: "BEST_CASE" as const, ownerId: user.id },
    { name: "Wipro Employee Security Awareness Deal", accountId: wiproAcc.id, amount: 450000, stageId: getDealStage("Proposal Sent") || getDealStage("Proposal"), probability: 50, forecastCategory: "PIPELINE" as const, ownerId: user.id },
  ...
  if (!seq1) {
    seq1 = await prisma.sequence.create({
      data: {
        ...
        steps: {
          create: [
            { order: 1, type: "EMAIL", config: { subject: "ISO27001 Compliance Audit for {{company}}", body: "Hi {{firstName}}, would love to share our security assessment framework..." } },
            { order: 2, type: "WAIT", config: { delayDays: 3 } },
            { order: 3, type: "CALL_REMINDER", config: { note: "Follow up call on security audit proposal" } },
            { order: 4, type: "EMAIL", config: { subject: "Case Study: How Reliance secured 5k endpoints", body: "Hi {{firstName}}, sharing a quick 2-page case study..." } },
          ],
        },
      },
    });
  ...
  // 12. Activities & Tasks (TasksPage & Action Center)
  const today = new Date();
  const tomorrow = new Date(Date.now() + 1 * 24 * 60 * 60 * 1000);
  const nextWeek = new Date(Date.now() + 5 * 24 * 60 * 60 * 1000);

  const activitiesData = [
    { type: "CALL" as const, subject: "Discovery Call with Rahul Sharma (HDFC CTO)", body: "Reviewed SIEM integration timelines. Rahul requested SOC2 report.", status: "COMPLETED" as const, objectType: "ACCOUNT" as const, accountId: hdfcAcc.id, ownerId: user.id, completedDate: new Date() },
  ...
  // 13. Notes (Shared Record Timeline)
  const notesData = [
    { body: "HDFC CTO confirmed executive board has allocated ₹5M budget for security upgrade in Q3.", accountId: hdfcAcc.id, authorId: user.id },
    ...
    { body: "Competitor bid is 10% lower, but our automated compliance reporting is a major advantage for Tata Cloud.", accountId: tataAcc.id, authorId: user.id },
  ];
  ...
  // 14. Forecast Targets (ForecastingPage)
  ...
  console.log(`✅ Tenant ${tenant.name} (${tenant.id}) successfully seeded!`);
}

async function main() {
  console.log("🚀 Seeding ALL tenants in database...");

  const tenants = await prisma.tenant.findMany();
  for (const tenant of tenants) {
    await seedTenant(tenant, null);
  }

  console.log("\n==================================================");
  console.log("🎉 ALL TENANTS AND USERS ARE NOW FULLY POPULATED!");
  console.log("==================================================\n");
}

main()
  .catch((e) => {
    console.error("❌ Error during global seed:", e);
    process.exit(1);
  })
  .finally(async () => {
