...
export default function OpportunitiesPage() {
  ...
  const { data, isLoading } = useQuery<Paginated<Opportunity>>({
    ...
    queryFn: async () => (await api.get("/opportunities", { params: { search, pageSize: 100, ...(stageId ? { stageId } : {}), ...(ownerId ? { ownerId } : {}) } })).data,
  });

  const bulkMutation = useMutation({
    mutationFn: (payload: any) => api.post("/opportunities/bulk", { ids: Array.from(selected), ...payload }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["opportunities"] }); setSelected(new Set()); setBulkOwnerPicker(false); setBulkStagePicker(false); },
  });
    ...
    <div>
      ...
      <div className="px-8 pb-8">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <div className="relative w-64">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search opportunities…" className={`${inputClass} pl-8`} style={inputStyle} />
          </div>
          <select value={stageId} onChange={(e) => setStageId(e.target.value)} className={inputClass} style={{ ...inputStyle, width: 180 }}>
        ...
        <div className="mb-4">
          <SavedViewsBar
            ...
            onApply={(f) => { setSearch(f.search || ""); setStageId(f.stageId || ""); setOwnerId(f.ownerId || null); setOwnerLabel(null); }}
          />
        </div>
        <Card>
          {isLoading ? (
            ...
            <table className="w-full text-sm">
              <thead><tr className="text-left border-b" style={{ borderColor: "var(--ink-100)" }}>
                ...
                {["Opportunity Name", "Account", "Contact Person", "Opportunity Stage", "Deal Stage", "Deal Value", "Assigned To", "Created Date", "Close Date"].map((h) => (
                  <th key={h} className="px-4 py-2.5 text-xs uppercase font-medium whitespace-nowrap" style={{ color: "var(--ink-400)" }}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {data.data.map((o) => {
                  const contactName = o.contact
                    ...
                    : (o.contacts && o.contacts[0]?.contact ? `${o.contacts[0].contact.firstName} ${o.contacts[0].contact.lastName}` : "—");

                  return (
                    <tr key={o.id} className="border-b last:border-0 hover:bg-[var(--ink-50)]" style={{ borderColor: "var(--ink-100)" }}>
                      <td className="px-4 py-3"><RowCheckbox checked={selected.has(o.id)} onChange={(v) => toggleOne(o.id, v)} /></td>
                      <td className="px-4 py-3"><Link to={`/opportunities/${o.id}`} className="font-semibold hover:underline" style={{ color: "var(--ledger-700)" }}>{o.name}</Link></td>
                      <td className="px-4 py-3 font-medium" style={{ color: "var(--ink-700)" }}>{o.account?.name || "—"}</td>
                      <td className="px-4 py-3" style={{ color: "var(--ink-600)" }}>{contactName}</td>
                      <td className="px-4 py-3"><StageBadge stage={o.stage as any} /></td>
                      <td className="px-4 py-3">
                        {o.dealStage ? <StageBadge stage={o.dealStage as any} /> : <span className="text-xs" style={{ color: "var(--ink-400)" }}>{o.stage.name}</span>}
                      </td>
                      ...
                      <td className="px-4 py-3 font-mono-num text-xs" style={{ color: "var(--ink-500)" }}>{formatDate(o.createdAt)}</td>
                      <td className="px-4 py-3 font-mono-num text-xs" style={{ color: "var(--ink-500)" }}>{formatDate(o.expectedCloseDate)}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {bulkOwnerPicker && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 w-72 p-3 rounded-lg border shadow-xl bg-white" style={{ borderColor: "var(--ink-100)" }}>
          <div className="text-xs font-medium mb-2" style={{ color: "var(--ink-500)" }}>Assign {selected.size} opportunit{selected.size === 1 ? "y" : "ies"} to</div>
          <RelationshipSelector
