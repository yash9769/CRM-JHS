      ...
      <div className="px-8 pb-10 space-y-6 max-w-4xl">
        ...
        <Card className="p-5">
          ...
          <dl className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><dt className="text-xs" style={{ color: "var(--ink-400)" }}>Your role</dt><dd className="mt-0.5"><Badge tone={roleTone[me?.orgRole || "MANAGER"]}>{roleLabel(me?.orgRole)}</Badge></dd></div>
          </dl>
        </Card>

        {/* Team — redirects to Org Chart for full management */}
        <div>
          ...
          <Card>
            <table className="w-full text-sm">
              <thead><tr className="text-left border-b" style={{ borderColor: "var(--ink-100)" }}>
                {["Member", "Role", "Open Deals", "Closed Won", canManage ? "Actions" : ""].map(h => (
