...
export default function ForecastingPage() {
  ...
  const attainmentPct = s?.attainment != null ? Math.round(s.attainment) : null;
      ...
      <div className="px-8 pb-10 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <Card className="p-4">
              <div className="flex items-center gap-2 mb-1.5">
                <AlertCircle size={14} style={{ color: attainmentPct != null && attainmentPct < 50 ? "var(--rose-500)" : "var(--ledger-500)" }} />
                <span className="text-xs" style={{ color: "var(--ink-400)" }}>Attainment</span>
              </div>
              <div className="font-mono-num text-2xl font-semibold" style={{ color: attainmentPct != null && attainmentPct < 50 ? "var(--rose-600)" : "var(--ledger-700)" }}>
                {attainmentPct != null ? `${attainmentPct}%` : "—"}
              </div>
              {s.gap > 0 && <div className="text-xs mt-0.5" style={{ color: "var(--rose-600)" }}>{formatCurrency(s.gap)} to go</div>}
            </Card>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          {forecast?.byOwner?.length > 0 && (
            <Card className="p-5">
              <h3 className="text-sm font-semibold mb-4" style={{ color: "var(--ink-800)" }}>By rep</h3>
              <table className="w-full text-sm">
                <thead><tr className="text-left border-b" style={{ borderColor: "var(--ink-100)" }}>
                  {["Rep", "Target", "Closed", "Attainment"].map(h => <th key={h} className="py-1.5 text-xs" style={{ color: "var(--ink-400)" }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {forecast.byOwner.map((row: any) => (
                    <tr key={row.owner.id} className="border-b last:border-0" style={{ borderColor: "var(--ink-50)" }}>
                      ...
                      <td className="py-2">
                        {row.attainment != null ? (
                          <span className="font-mono-num" style={{ color: row.attainment >= 100 ? "var(--ledger-700)" : row.attainment >= 50 ? "var(--amber-600)" : "var(--rose-600)" }}>
