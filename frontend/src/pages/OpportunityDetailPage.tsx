...
import { Card, Button, Badge, Modal, Field, inputClass, inputStyle, StageBadge } from "../components/ui";
import { Timeline } from "../components/Timeline";
import { NewContactModal, NewAccountModal, NewDealModal, NewTaskModal, LogActivityModal } from "../components/CreateModals";
...
import {
  ArrowRight, CheckCircle2, Building2, User, Pencil, UserPlus, CheckSquare,
  PhoneCall, Archive, Handshake,
} from "lucide-react";
  ...
  return (
    <Modal title="Convert to Deal" onClose={onClose}>
      <p className="text-sm mb-4" style={{ color: "var(--ink-600)" }}>
        ...
        The account, contact person, deal value, and owner carry over — this opportunity remains on record as the source.
      </p>
      <div className="space-y-4">
        ...
        {mutation.isError && <div className="text-sm mb-3 text-[var(--rose-600)]">Could not convert this opportunity.</div>}
        <div className="flex justify-end gap-2 pt-2 border-t" style={{ borderColor: "var(--ink-100)" }}>
          <Button variant="secondary" onClick={onClose} type="button">
            Cancel
          </Button>
          <Button onClick={() => mutation.mutate()} disabled={mutation.isPending}>
            {mutation.isPending ? "Converting…" : "Convert to Deal"}
          </Button>
        </div>
      </div>
    </Modal>
...
export default function OpportunityDetailPage() {
  ...
  const [modal, setModal] = useState<"edit" | "contact" | "account" | "deal" | "lead" | "task" | "log" | "archive" | null>(null);
    ...
    <div className="px-8 py-7 max-w-6xl">
        ...
        <div className="col-span-2 space-y-5">
          <Card className="p-5">
            ...
            <dl className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm">
              ...
              <div>
                <dt className="text-xs font-medium uppercase tracking-wider" style={{ color: "var(--ink-400)" }}>Deal Stage</dt>
                <dd className="mt-1">
                  {opp.dealStage ? (
                    <StageBadge stage={opp.dealStage as any} />
                  ) : (
                    <span className="text-xs" style={{ color: "var(--ink-500)" }}>{opp.stage.name}</span>
                  )}
                </dd>
              </div>
      ...
      {modal === "deal" && (
        <NewDealModal
          ...
          contactName={contactPerson ? `${contactPerson.firstName} ${contactPerson.lastName}` : undefined}
          initialAmount={opp.amount}
          initialRemarks={opp.description || undefined}
          onClose={() => setModal(null)}
          onCreated={(d) => navigate(`/deals/${d.id}`)}
        />
      )}
      {modal === "task" && (
        <NewTaskModal
