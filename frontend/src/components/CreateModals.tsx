...
export function NewOpportunityModal({
  ...
  // Queries for Opportunity and Deal pipelines
  ...
  const effectiveDealStageId = form.dealStageId || dealPipeline?.stages[0]?.id || "";

  // Auto-sync stage index between Opportunity Stage and Deal Stage if user hasn't explicitly set dealStageId
  function handleOpportunityStageChange(newStageId: string) {
    const oppStages = oppPipeline?.stages || [];
    const dealStages = dealPipeline?.stages || [];
    const idx = oppStages.findIndex((s) => s.id === newStageId);
    const matchingDealStage = idx !== -1 && dealStages[idx] ? dealStages[idx].id : form.dealStageId;

    setForm((prev) => ({
      ...prev,
      stageId: newStageId,
      dealStageId: matchingDealStage || prev.dealStageId,
    }));
  }
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return api.post("/opportunities", {
        ...
        dealStageId: effectiveDealStageId || null,
        ownerId: ownerId,
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Opportunity Stage" required>
              <select
                ...
                onChange={(e) => handleOpportunityStageChange(e.target.value)}
                className={inputClass}
                style={inputStyle}
              >
