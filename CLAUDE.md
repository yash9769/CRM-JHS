...
- **Database Migrations / Schema Sync**:
  ...
  > **Warning — pre-`orgRole` databases**: `backend/prisma/migrations/20260910120000_sync_drifted_schema/` reconciles the migration history with `schema.prisma` after a long run of changes (audit `createdById` columns, `Role` → `OrgRole`, `users.partnerId`, `opportunities.contactId`, new money fields) that were applied via `db push` and never captured as migrations. It **drops** the old `users.role` column outright. If a database still has that column with real data, do not run this migration (or `migrate deploy`) against it as-is — either keep using `db push` there, or migrate `role` → `orgRole` by hand first. Safe to run unmodified only against an empty/fresh database (e.g. a brand-new staging/production deploy). This migration also adds the `stage_approvals` table and `ApprovalStatus` enum, which were missing from migration history entirely (added in `20260902120000_add_sticky_notes_and_close_details/` instead, alongside two other fixed bugs there — see that file's comments).
- **Seed Demo Data**:
  ```bash
